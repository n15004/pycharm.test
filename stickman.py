...
class Game:
    def __init__(self):
        ...
        self.bg = PhotoImage(file="stickman/background2.gif")
        w = self.bg.width()
        h = self.bg.height()
        for x in range(0, 5):
            for y in range(0, 5):
        ...
        self.sprites = []
        self.running = True
        self.game_over_text = self.canvas.create_text(250, 250, text='Junk GameCLEAR Happy', state='hidden')
...
class DoorSprite(Sprite):
    def __init__(self, game, photo_image, x, y, width, height):
        Sprite.__init__(self, game)
        self.closed_door = PhotoImage(file="stickman/door1.gif")
        self.open_door = PhotoImage(file="stickman/door2.gif")
        self.photo_image = photo_image
        self.image = game.canvas.create_image(x, y, image=self.photo_image, anchor='nw')
...
if __name__ == '__main__':
    ...
    # use move class
    platform5 = MovingPlatformSprite(g, PhotoImage(file='stickman/platform2.gif'), 175, 360, 66, 10)
    ...
    door = DoorSprite(g, PhotoImage(file="stickman/door1.gif"), 45, 50, 40, 35)
